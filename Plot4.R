...
plot(strptime(paste(HPC[,1],HPC[,2]), "%Y-%m-%d %H:%M:%S"), HPC$Voltage, xlab="datetime",ylab="Voltage",type="l")
plot(strptime(paste(HPC[,1],HPC[,2]), "%Y-%m-%d %H:%M:%S"), HPC$Global_reactive_power, xlab="datetime",ylab="Global_reactive_power",type="l")
dev.off()
